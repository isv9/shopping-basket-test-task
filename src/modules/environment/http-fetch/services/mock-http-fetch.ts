...
function getShoppingMenu(): Promise<HttpFetchResponse<any>> {
    const response: HttpFetchResponse<Array<ShoppingMenuItem>> = {
        data: [
            'Четыре сыра',
            'Ветчина и грибы',
            'Маргарита',
            ...
            'Пепперони',
            'Итальянская'
        ].map((name, index): ShoppingMenuItem => {
            return {
                name,
                id: (index + 1).toString()
            }
...
function addOrderedItem(shoppingBasket: ServiceShoppingBasketDTO,
                        data: AddingOrderedItemToShoppingBasketInputDTO): Promise<HttpFetchResponse<any>> {
    const orderedItemId = data.orderedItemId;
    shoppingBasket.orderedItems = [
        ...shoppingBasket.orderedItems,
        {itemId: orderedItemId, count: 1}
    ];
...
function adjustOrderedItem(shoppingBasket: ServiceShoppingBasketDTO,
                           requestParams: AdjustingOrderedItemInShoppingBasketInputDTO): Promise<HttpFetchResponse<any>> {
    const orderedItemId = requestParams.orderedItemId;
    const orderedItem = shoppingBasket.orderedItems
        .find(orderedItem => orderedItem.itemId === orderedItemId);
    if (!orderedItem) {
    ...
    if ((orderedItem.count + adjustmentValue) <= 0) {
        shoppingBasket.orderedItems = shoppingBasket.orderedItems
            .filter(orderedItem => orderedItem.itemId !== orderedItemId);
    } else {
        shoppingBasket.orderedItems = shoppingBasket.orderedItems
            .map(orderedItem => {

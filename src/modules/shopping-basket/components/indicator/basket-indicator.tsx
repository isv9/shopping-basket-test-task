...
export const ShoppingBasketIndicator: React.FC<ShoppingBasketIndicatorProps> = (props) => {
    ...
    return (<Link to={urlConfig.shoppingBasket} style={styles.container}>
        <span className={css(styles.shoppingBasketLabel)}>Basket</span>
        {shoppingBasket.hasOrderedItems && <div className={css(styles.orderItemsCountContainer)}>
            <span className={css(styles.orderItemsCountLabel)}>
                {shoppingBasket.orderedItemsCount}
            </span>
        </div>}
    </Link>);
};
